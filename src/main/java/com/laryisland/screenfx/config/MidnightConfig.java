			...
			if (this.list != null) {
				for (ButtonEntry entry : this.list.children()) {
					if (entry.buttons != null && entry.buttons.size() > 1 && entry.buttons.get(1) instanceof ButtonWidget button) {
						var prev_active = button.active;
						button.active = !Objects.equals(entry.info.value.toString(), entry.info.defaultValue.toString());
						if (button.active != prev_active) {
							if (button.active) {
								button.setMessage(new LiteralText(button.getMessage().getString()).formatted(Formatting.WHITE));
							} else {
								button.setMessage(
										new LiteralText(button.getMessage().getString()).formatted(Formatting.GRAY));
							}
						}
					}
				}
		...
		public void init() {
			...
			for (EntryInfo info : entries) {
				if (info.id.equals(modid)) {
					...
					ButtonWidget resetButton = new ButtonWidget(width - 205, 0, 40, 20, new LiteralText("Reset").formatted(Formatting.WHITE), (button -> {
					...
					if (info.widget instanceof Map.Entry) {
						...
						resetButton.setMessage(new LiteralText("R").formatted(Formatting.WHITE));
