...
public abstract class MidnightConfig {
	...
	public static class MidnightConfigScreen extends Screen {
		...
		public Tooltip getTooltip(EntryInfo info) {
			return Tooltip.of(I18n.hasTranslation(translationPrefix + info.field.getName() + ".tooltip") ? Text.translatable(
					translationPrefix + info.field.getName() + ".tooltip") : Text.empty());
		}
				...
				if (info.id.equals(modid)) {
					...
					if (info.widget instanceof Map.Entry) {
						...
						if (info.field.getType().isEnum()) {
							widget.setValue(value -> Text.translatable(
									translationPrefix + "enum." + info.field.getType().getSimpleName() + "." + info.value.toString()));
						}
						this.list.addButton(List.of(ButtonWidget.builder(widget.getValue().apply(info.value), widget.getKey())
								.dimensions(width - 160, 0, 150, 20).tooltip(getTooltip(info)).build(), resetButton), name, info);
						...
						Predicate<String> processor = ((BiFunction<TextFieldWidget, ButtonWidget, Predicate<String>>) info.widget).apply(
								widget, done);
						...
						if (e.isSlider()) {
							widget = new MidnightSliderWidget(width - 160, 0, 150, 20, Text.of(info.tempValue),
			...
			if (text != null && (!text.getString().contains("spacer") || !buttons.isEmpty())) {
				if (info.centered) {
					textRenderer.drawWithShadow(matrices, text,
							MinecraftClient.getInstance().getWindow().getScaledWidth() / 2f - (textRenderer.getWidth(text) / 2f),
							y + 5, 0xFFFFFF);
				} else {
	...
	private static class MidnightSliderWidget extends SliderWidget {
		...
		protected void applyValue() {
			if (info.field.getType() == int.class) {
				...
				info.value =
						Math.round((e.min() + value * (e.max() - e.min())) * (double) e.precision()) / (double) e.precision();
			} else if (info.field.getType() == float.class) {
				info.value =
						Math.round((e.min() + value * (e.max() - e.min())) * (float) e.precision()) / (float) e.precision();
			}
