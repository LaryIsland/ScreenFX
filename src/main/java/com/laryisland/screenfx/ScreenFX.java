...
public class ScreenFX implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ScreenFXConfig.ON_CLICK_LISTENERS.add(id -> {
			if (Objects.equals(id, "elderGuardianTesting")) {
				LocalPlayer player = Minecraft.getInstance().player;
				if (player != null && Minecraft.getInstance().level != null) {
					Minecraft.getInstance().level.addParticle(ParticleTypes.ELDER_GUARDIAN,
						player.getX(), player.getY(), player.getZ(), 0.f, 0.f, 0.f
					);
				}
				ScreenFXConfig.elderGuardianTesting = false;
			}
		});
	}
}